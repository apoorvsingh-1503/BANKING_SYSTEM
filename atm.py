...
def add_beneficiary_to_file():
    ...
    with open(File_Beneficiary, "a") as f:
        f.write(f"{b_name},{b_acc},{b_limit}\n")
...
def print_beneficiary_file():
    ...
    with open(File_Beneficiary, "r") as f:
        for idx, line in enumerate(f, 1):
            name, acct, limit = line.strip().split(",")
...
def account_creation():
    ...
    with open(File_Transaction, "w") as f:
        f.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S Account Created.')}]\n")
...
def login():
    ...
    ehs_pin = input("ENTER YOUR HIGH SEQURITY PIN : ")
    if ehs_pin==data["hs_pin"]:
        print("SEQURITY VERIFICATION PASSED")
        dashboard(data)
        return True
    else:
        ...
        if ans1 == data["a1"] and ans2 == data["a2"] and ans3 == data["a3"]:
            ...
            print("SEQURITY VERIFICATION FAILED ! DELETING ALL LOGS")
            file_delete()
            return False
...
def dashboard(data):
    while True:
        ...
        if c==1:
            hs=input("ENTER HIGH SEQURITY PIN : ")
            if hs==data["hs_pin"]:
                add_beneficiary_to_file()
            else:
                print("INCORRECT HIGH SEQURITY PIN ENTERED")
        elif c==2:
            ...
            if amount > current_data["balance"]:
                print("TRANSACTION CANNOT BE PROCCED. INSUFFICIENT BALANCE")
            else:
                tp = input("Enter TPIN: ")
